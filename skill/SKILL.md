...
license: Apache-2.0
homepage: https://github.com/danielithomas/outclaw
user-invocable: true
compatibility: Requires Python 3.9+, network access to graph.microsoft.com, and one-time OAuth setup
metadata:
  author: Daniel Thomas
  version: "1.0.0"
  openclaw: {"requires": {"anyBins": ["python", "python3", "outclaw"]}, "os": ["darwin", "linux", "win32"]}
---
...
2. **OAuth Consent** - Run `outclaw auth login` and approve permissions
...
outclaw mail list --unread

# Get specific message
outclaw mail get <message-id>

# Send email
outclaw mail send --to user@example.com --subject "Hello" --body "Message text"
...
# JSON output
outclaw --json calendar list --start 2026-02-01 --end 2026-02-28
```
...
outclaw tasks list --list-id <list-id>

# List only active (not completed) tasks
outclaw tasks list --list-id <list-id> --status active

# Create task
outclaw tasks create --list-id <list-id> --title "Complete report" --due-date "2026-02-20"

# Mark task complete
outclaw tasks complete --list-id <list-id> --task-id <task-id>

# Reopen a completed task
outclaw tasks reopen --list-id <list-id> --task-id <task-id>
```
...
|-------|-------|----------|
| `AuthenticationError` | Not logged in or token expired | Run `outclaw auth login` |
...
[Runs: outclaw --json mail list --limit 20]
...
[Runs: outclaw calendar create --subject "Meeting with John" 
...
[Runs: outclaw --json tasks list --list-id <id>]
[Runs: outclaw tasks complete --list-id <id> --task-id <id>]
...
1. **Check authentication**: Run `outclaw auth status`
2. **Re-authenticate**: Run `outclaw auth login`
3. **Verify network**: Ensure `graph.microsoft.com` is reachable
4. **Check environment**: Verify OUTCLAW_CLIENT_ID is set

## References

- [Outclaw Documentation](https://github.com/danielithomas/outclaw)
